...
DODO_FEE_ROUTE_PROXY = Web3.to_checksum_address("0x8c6DbF95448AcbcBb1c3D6E9b3b9ceF7E6fbAb00")
...
LANG = {
    'vi': {
        'title': 'GASPUMP SWAP - RISE TESTNET',
        'info': 'Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
        ...
        'action_prompt': 'Chọn hành động [1-4]',
        'invalid_choice': 'Lựa chọn không hợp lệ, vui lòng chọn từ 1-4',
    ...
    'en': {
        ...
        'action_prompt': 'Select action [1-4]',
        'invalid_choice': 'Invalid choice, please select from 1-4',
...
def check_balance(w3: Web3, address: str, token_address: str, decimals: int, language: str = 'en') -> float:
    if token_address == "native":
        ...
        token_abi = WETH_ABI if token_address == WETH_ADDRESS else USDC_ABI
...
def display_token_balances(w3: Web3, address: str, language: str = 'en'):
    ...
    usdc_balance = check_balance(w3, address, USDC_ADDRESS, 6, language)
    print(f"{Fore.YELLOW}  - ETH: {eth_balance:.6f}{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}  - WETH: {weth_balance:.6f}{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}  - USDC: {usdc_balance:.6f}{Style.RESET_ALL}")
...
async def execute_action(w3: Web3, private_key: str, wallet_index: int, action: int, amount_in: float, times: int, language: str = 'en'):
    ...
    dodo_contract = w3.eth.contract(address=DODO_FEE_ROUTE_PROXY, abi=DODO_FEE_ROUTE_PROXY_ABI)
    ...
    for i in range(times):
        ...
        usdc_balance = check_balance(w3, sender_address, USDC_ADDRESS, 6, language)
        
        amount_wei = int(amount_in * (10 ** 18 if action in [1, 2, 3] else 6))
        ...
        if action == 1 and eth_balance < amount_in:
            ...
            break
        elif action == 3 and weth_balance < amount_in:
            print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance']}: {weth_balance:.6f} WETH{Style.RESET_ALL}")
            break
        elif action == 4 and usdc_balance < amount_in:
            print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance']}: {usdc_balance:.6f} USDC{Style.RESET_ALL}")
            break
        ...
        try:
            if action == 1:  # Wrap ETH → WETH
                ...
                tx['gas'] = w3.eth.estimate_gas(tx) if w3.eth.estimate_gas(tx) else 95312
            elif action == 2:  # Unwrap WETH → ETH
                ...
                tx['gas'] = w3.eth.estimate_gas(tx) if w3.eth.estimate_gas(tx) else 95312
            elif action in [3, 4]:  # Swap WETH → USDC or USDC → WETH
                token_address = WETH_ADDRESS if action == 3 else USDC_ADDRESS
                if not await approve_token(w3, private_key, token_address, DODO_FEE_ROUTE_PROXY, amount_wei, language, nonce):
                    break
                nonce += 1
                from_token = WETH_ADDRESS if action == 3 else USDC_ADDRESS
                to_token = USDC_ADDRESS if action == 3 else WETH_ADDRESS
                usdc_per_weth = 1071.568
                exp_return = amount_in * usdc_per_weth if action == 3 else amount_in / usdc_per_weth
                exp_return_wei = int(exp_return * (10 ** 6 if action == 3 else 18))
                min_return_wei = int(exp_return_wei * 0.968)
                tx = dodo_contract.functions.mixSwap(
                    from_token,
                    to_token,
                    amount_wei,
                    exp_return_wei,
                    ...
                    [Web3.to_checksum_address("0xc7E2B7C2519bB911bA4a1eeE246Cb05ACb0b1df1")],
                    [Web3.to_checksum_address("0xc7E2B7C2519bB911bA4a1eeE246Cb05ACb0b1df1"), DODO_FEE_ROUTE_PROXY],
                    0 if action == 3 else 1,
                    ["0x00"],
                ...
                tx['gas'] = w3.eth.estimate_gas(tx) if w3.eth.estimate_gas(tx) else 300000
            ...
            if receipt.status == 1:
                ...
                usdc_balance_after = check_balance(w3, sender_address, USDC_ADDRESS, 6, language)
                ...
                print(f"{Fore.YELLOW}    {LANG[language]['balance']:<12}: {eth_balance_after:.6f} ETH | WETH: {weth_balance_after:.6f} | USDC: {usdc_balance_after:.6f}{Style.RESET_ALL}")
            else:
...
async def run_gaspump(language: str = 'en'):
    print()
    ...
    for i, (profile_num, private_key) in enumerate(private_keys, 1):
        ...
        print()
        while True:
            print(f"{Fore.CYAN}Select action [1-4]:{Style.RESET_ALL}")
            try:
        ...
        usdc_balance = check_balance(w3, account.address, USDC_ADDRESS, 6, language)
        max_amount = eth_balance if action == 1 else weth_balance if action in [2, 3] else usdc_balance

        print()
        while True:
            print(f"{Fore.CYAN}{LANG[language]['amount_prompt']} {Fore.YELLOW}(Max: {max_amount:.4f} {'ETH' if action == 1 else 'WETH' if action in [2, 3] else 'USDC'}){Style.RESET_ALL}")
            try:
