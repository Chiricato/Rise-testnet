...
LANG = {
    'vi': {
        'title': '🌸 ĐĂNG KÝ WAITLIST NOVADUBS 🌸',
        'info': 'Thông tin',
        'found': 'Tìm thấy',
        'emails': 'email',
        'valid': 'hợp lệ',
        ...
        'success': 'Thành công: {email} đã đăng ký với tên {name}!',
        'failure': 'Thất bại: {email} - Mã trạng thái: Mail {mail_code}, Notion {notion_code}',
    ...
    'en': {
        'title': '🌸 NOVADUBS WAITLIST REGISTRATION 🌸',
        'info': 'Info',
        'found': 'Found',
        'emails': 'emails',
        'valid': 'valid',
...
def join_waitlist(email: str, language: str = 'en') -> bool:
    ...
    try:
        ...
        if mail_res.status_code == 200 and notion_res.status_code == 200:
            print(f"{Fore.GREEN}  ✔ {LANG[language]['success'].format(email=email, name=name)}{Style.RESET_ALL}")
            return True
        else:
            print(f"{Fore.RED}  ✖ {LANG[language]['failure'].format(email=email, mail_code=mail_res.status_code, notion_code=notion_res.status_code)}{Style.RESET_ALL}")
            return False
    except Exception as e:
