...
# Constants
NETWORK_URLS = ['https://testnet.riselabs.xyz']
CHAIN_ID = 11155931
...
LANG = {
    'vi': {
        'title': '✨ GỬI GIAO DỊCH - RISE TESTNET ✨',
        'info': 'ℹ Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
        ...
        'connect_success': '✅ Thành công: Đã kết nối mạng Rise Testnet',
    ...
    'en': {
        'title': '✨ SEND TRANSACTION - RISE TESTNET ✨',
        'info': 'ℹ Info',
        'found': 'Found',
        'wallets': 'wallets',
...
async def send_transaction(w3: Web3, private_key: str, to_address: str, amount: float, wallet_index: int, tx_index: int, total_tx: int, language: str = 'en'):
    ...
    try:
        ...
        try:
            ...
            gas_limit = 21000  # Gas mặc định cho giao dịch ETH đơn giản
        ...
        if balance < required_balance:
            print(f"{Fore.RED}  ✖ Số dư không đủ: {balance:.6f} ETH (Cần: {required_balance:.6f} ETH){Style.RESET_ALL}")
            return False
